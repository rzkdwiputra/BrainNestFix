...
export const updateAccesstoken = CatchAsyncError(async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    await redis.set(user._id, JSON.stringify(user), "EX", 6040800); //7days
...
export const updatePassword = CatchAsyncError(async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (!oldPassword || !newPassword) {
      // Perbaikan 1: Penyesuaian kondisi
    ...
    if (!user || user.password === undefined) {
      // Perbaikan: Tambahkan pengecekan apakah user ditemukan
    ...
    // Tidak perlu menambahkan ': any' pada 'error', karena 'catch' akan menangkap segala jenis kesalahan
    return next(new ErrorHandler(error.message, 400));
  }
});
...
export const updateUserRole = CatchAsyncError(async (req: Request, res: Response, nest: NextFunction) => {
  try {
