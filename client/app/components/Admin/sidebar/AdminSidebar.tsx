      ...
      <ProSidebar collapsed={isCollapsed} style={{ position: "fixed", top: 0, left: 0, height: "100vh", width: isCollapsed ? "0%" : "16%" }}>
        <Menu iconShape="square">
          ...
          >
            {!isCollapsed && (
              <Box display="flex" justifyContent="space-between" alignItems="center" ml="15px">
                <Link href="/">
                  <h3 className="text-[25px] font-Poppins uppercase dark:text-white text-black ">BrainNest</h3>
                </Link>
          ...
          {!isCollapsed && (
            <Box mb="25px">
              ...
              <Box textAlign="center">
                <Typography variant="h4" className="!text-[20px] text-black dark:text-white" sx={{ m: "10px 0 0 0" }}>
                  {user?.name}
                </Typography>
                <Typography variant="h6" sx={{ m: "10px 0 0 0" }} className="!text-[20px] text-black dark:text-white capitalize ">
                  - {user?.name}
                </Typography>
              </Box>
            </Box>
          ...
          <Box paddingLeft={isCollapsed ? undefined : "10%"}>
            ...
            <Typography variant="h5" sx={{ m: "15px 0 15px 25px" }} className="!text-[18px] text-black dark:text-white capitalize !font-[400] ">
              {!isCollapsed && "Data"}
            </Typography>
            ...
            <Item title="Create Courses" to="/admin/create-course" icon={<VideoCallIcon />} selected={selected} setSelected={setSelected} />
            <Item title="Live Courses" to="/admin/courses" icon={<OndemandVideoIcon />} selected={selected} setSelected={setSelected} />
            <Typography variant="h5" className="!text-[18px] text-black dark:text-white capitalize !font-[400] " sx={{ m: "15px 0 5px 20px" }}>
            ...
            <Typography variant="h6" className="!text-[18px] text-black dark:text-white capitalize !font-[400] " sx={{ m: "15px 0 5px 20px" }}>
              {!isCollapsed && "Analitycs"}
            </Typography>
            ...
            <div onClick={logoutHandler}>
              <Item title="Back " to="/" icon={<ExitToAppIcon />} selected={selected} setSelected={setSelected} />
            </div>
          </Box>
        </Menu>
      </ProSidebar>
